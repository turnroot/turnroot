...
public class TestGenericEnemySkewWindow : EditorWindow
{
    ...
    private readonly int[] seeds = new[] { 131662290, 123456789, 588400120, 737628147, 300104262 };
    ...
    private void OnGUI()
    {
        ...
        EditorGUILayout.LabelField("Generic Enemy Skew Adjustment Range", EditorStyles.boldLabel);
    ...
    private static double DeterministicDouble(double min, double max, int battleSeed, string salt)
    {
        unchecked
        {
            int seed =
    ...
    )
    {
        ...
        sb.AppendLine($"  mean = {mean:0.##}, difference vs player mean = {diff:+0.##;-0.##;0.00}");
