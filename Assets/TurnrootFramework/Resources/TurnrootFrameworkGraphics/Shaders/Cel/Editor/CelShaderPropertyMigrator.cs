...
public class CelShaderPropertyMigrator : EditorWindow
{
    ...
    private static readonly Dictionary<string, string> TextureMap = new Dictionary<string, string>
    {
        { "_MainTex", "_BaseMap" },
        { "_NormalMap", "_BumpMap" },
        { "_matcap", "_MatcapTex" },
    ...
    private static readonly Dictionary<string, float> DroppedFloatsWithOldDefault = new Dictionary<
        string,
        float
    >
    {
    ...
    private void OnGUI()
    {
        ...
        _dryRun = EditorGUILayout.ToggleLeft("Dry Run (log only, don't modify materials)", _dryRun);
    ...
    )
    {
        ...
        {
            if (oldFloats.TryGetValue(kv.Key, out float val) && !Mathf.Approximately(val, kv.Value))
            {
                Debug.LogWarning(
