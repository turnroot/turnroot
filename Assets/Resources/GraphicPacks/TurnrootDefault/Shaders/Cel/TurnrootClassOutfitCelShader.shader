    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct Attributes
            {
                ...
                float4 color : COLOR; // vertex color (used for skin/accents selection)
            };
            ...
            Varyings vert (Attributes input)
            {
                ...
                // pass through vertex color (may be absent on some meshes)
            ...
            {
                // Alpha clip from base texture alpha (use _Base alpha)
                ...
                // --- TintMask + vertex-color precompute
                // TintMask R/G/B selects Accent1/2/3 replacement; if vertex color is (nearly) white use Skin color instead
                float4 tintMask = SAMPLE_TEXTURE2D(_Tint_Mask, sampler_Tint_Mask, TRANSFORM_TEX(input.uv, _Tint_Mask));
                float3 vcol = input.color.rgb;
                // treat vertex color as "use skin" when all channels are very near white
                float useSkin = step(0.99, min(min(vcol.r, vcol.g), vcol.b));
                ...
                float3 computed = baseTex.rgb;
                computed = lerp(computed, tintColor1, tintMask.r);
                computed = lerp(computed, tintColor2, tintMask.g);
                computed = lerp(computed, tintColor3, tintMask.b);

                baseTex.rgb = computed;

                // Optional: sample MSE map (if present) and write into albedo's alpha for downstream debugging / usage
                // (we only expose the texture so CharacterClassDataInstance can assign it; actual M/S/E handling is in the material/shader graph)
                #if defined(_MSE)
                    // nothing required here — _MSE is available as a property for tools/graph compatibility
                #endif
        ...
        Pass
        {
            ...
            {
                float2 uv_Base = TRANSFORM_TEX(input.uv, _Base);
                float4 baseTexShadow = SAMPLE_TEXTURE2D(_Base, sampler_Base, uv_Base);
